...
function getMarketType(market, exchange) {
	switch (exchange) {
		...
		case "betfair":
			if (market.eventType.name === "Soccer") {
				if (market.description.marketType.includes("HANDICAP") || market.marketName === "Goal Lines" || market.marketName.includes("+")) {
					return "HANDICAP";
				}
				if (market.marketName.includes("Over/Under")) {
					return "TOTAL";
				}
		...
		default:
			return "Oh no!";
		// if (market.description.bettingType.includes("ASIAN")) {
		// 	return "HANDICAP";
		// } else if (market.description.bettingType === "ODDS") {
		// 	if (market.marketName === "Match Odds" || market.marketName === "Moneyline") {
		// 		return "MONEYLINE";
		// 	} else {
		// 		return "TOTAL";
		// 	}
