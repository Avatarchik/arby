...
export default class AccountsAPI {
    constructor() {
        ...
        forEach(EnumSchemas, (Enum, id) =>
            this._validator.addSchema(Enum, Enum.id)
        );
        forEach(OperationSchemas, (Operation, id) =>
            this._validator.addSchema(Operation, Operation.id)
        );
    }
    ...
    executeRequest(operation, filters) {
        return this._api.post(
            `/${process.env.BETFAIR_ACCOUNT_JSONRPC_ENDPOINT}`,
            {
                data: {
                    jsonrpc: "2.0",
    ...
    validateParams(reqName, params) {
        let validation = this._validator.validate(
            params,
            OperationSchemas[reqName]
        );

        if (validation.error && validation.errors.length) {
            throw `${reqName} errors; ${this.formatValidationErrors(
                validation.errors
            )}`;
        }
    }
